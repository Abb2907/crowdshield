...
exports.app.post('/api/auth/login', (req, res) => {
    ...
    if (username === 'admin' && password === 'admin123') {
        const token = jsonwebtoken_1.default.sign({ username: 'admin', role: 'admin' }, JWT_SECRET, { expiresIn: '2h' });
        return res.json({ success: true, token, user: { username: 'admin', role: 'admin' } });
    }
    else if (username === 'operator' && password === 'operator123') {
        const token = jsonwebtoken_1.default.sign({ username: 'operator', role: 'operator' }, JWT_SECRET, { expiresIn: '2h' });
        return res.json({ success: true, token, user: { username: 'operator', role: 'operator' } });
    }
    else if (username === 'viewer' && password === 'viewer123') {
        const token = jsonwebtoken_1.default.sign({ username: 'viewer', role: 'viewer' }, JWT_SECRET, { expiresIn: '2h' });
...
exports.app.get('/api/stadium', (req, res) => {
    res.json({ zones, gates, incidents, alerts, evacuationState });
});
exports.app.post('/api/emergency', authenticateJWT, requireRole(['admin', 'operator']), async (req, res) => {
...
exports.app.post('/api/incident', authenticateJWT, requireRole(['admin', 'operator']), async (req, res) => {
...
exports.app.post('/api/gate/status', authenticateJWT, requireRole(['admin', 'operator']), async (req, res) => {
...
exports.app.post('/api/ai/briefing', authenticateJWT, requireRole(['admin', 'operator', 'viewer']), async (req, res) => {
...
exports.app.post('/api/ai/chat', async (req, res) => {
...
exports.app.post('/api/ai/voice', authenticateJWT, requireRole(['admin', 'operator']), async (req, res) => {
    ...
    if (cmdLower.includes('emergency') || cmdLower.includes('evacuate') || cmdLower.includes('evacuation')) {
        ...
        let status = 'open';
        if (cmdLower.includes('close') || cmdLower.includes('lock'))
            status = 'closed';
        else if (cmdLower.includes('restrict'))
            status = 'restricted';
        let targetGateId = '';
        if (cmdLower.includes('gate a') || cmdLower.includes('gate_a') || cmdLower.includes('stand a'))
            targetGateId = 'gate_a';
        else if (cmdLower.includes('gate b') || cmdLower.includes('gate_b') || cmdLower.includes('stand b'))
            targetGateId = 'gate_b';
        else if (cmdLower.includes('gate c') || cmdLower.includes('gate_c') || cmdLower.includes('stand c'))
            targetGateId = 'gate_c';
        else if (cmdLower.includes('gate d') || cmdLower.includes('gate_d') || cmdLower.includes('stand d'))
            targetGateId = 'gate_d';
        if (targetGateId) {
...
exports.app.post('/api/ticket/scan', (req, res) => {
...
exports.app.get('/api/analytics', (req, res) => {
